...
public class InternObserver implements Observer{
    private List<Job> appliedList;
    private Dashboard dashboard;

    public InternObserver(Dashboard dashboard) {
    ...
    public void update(Job job) {
        System.out.println("Intern received a new job alert.");
        apply(job);
    }

    @Override
    public void apply(Job job) {
        if(job.getJobType().equals(JobType.INTERNSHIP)) {
            appliedList.add(job);
        }
    }
